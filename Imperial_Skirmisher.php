<!DOCTYPE html>
<html lang="zh-tw">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="description" content="">
    <meta name="author" content="">
    <link rel="icon" href="favicon.ico">
    <title>Aoetw</title>
    <style>
      body {            padding-top: 0px;            font-family: "微軟正黑體";            background-color: #f5f5f5;            font-size: 18px;        }
    </style>
  </head>
  <body>
    <?php include( "head.php"); ?>
    <!-- Main jumbotron for a primary marketing message or call to action
    -->
    <div class="container">
      <!-- Example row of columns -->
      <div class="row" style=" margin-top: 50px;">
        <div class="col-md-12">
          <h1>帝王戰矛兵</h1>
          <div class="hrn1"></div>
        </div>
        <div class="col-md-8">
          <div class="n1">
            <p>帝王戰矛兵是<a href="Vietnamese.php">越南</a>第二個特殊單位，可以在 <a href="Archery_Range.php">射箭場</a> 訓練，是戰矛兵的升級，是一個反弓箭手的遠程單位。</p>
            <p>帝王戰矛兵是一種共同的特殊單位，只要是越南或與他的同盟玩家都可以使用（<a href="Turks.php">土耳其人</a>例外）。</p>
          </div>
          <h3>戰術</h3>
          <div class="hrn1"></div>
          <div class="n1">
            <p>除了對付弓箭手、長槍兵與其他遠程單位外，帝王戰矛兵顯得非常虛弱，他們攻擊、射速都低還有最小射程，對步兵及騎兵單位難以有效的進行傷害，但非常克制弓箭手。</p>
            <p>由於帝王戰矛兵生產不用黃金，所以是一個垃圾兵單位，在缺少黃金時期，是非常重要的兵種，由於成本低、訓練時間短，在某些情況下，可以用數量壓制對手。</p>
            <p>帝王戰矛兵對槍兵有額外的傷害，雙方數量少時，槍兵將獲勝，但數量一多起來，戰矛兵將克制槍兵。</p>
            <p>在垃圾兵大戰中，帝王戰矛兵非常受歡迎。但要注意的是由於帝王戰矛兵也算一個普通單位，日本武士對他沒有加成。</p>
            <p>不幸的是，<a href="Turks.php">土耳其</a>並不能使用帝王戰矛兵，因為土耳其人連戰矛兵都沒有，相對之下，有些文明的帝王戰矛兵非常有優勢，特別是
              <a
              href="Aztecs.php">阿茲特克</a>、<a href="Britons.php">不列顛</a>、<a href="Byzantines.php">拜占庭</a>和
                <a
                href="Incas.php">印加</a>。</p>
          </div>
          <h3>帝王戰矛兵介紹影片</h3>
          <div class="hrn1"></div>
          <div class="n1">
            <p><em>作者：Spirit Of The Law</em>
            </p>
            <iframe width="560" height="315" src="https://www.youtube.com/embed/Zk9qKhklvcI"
            frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe>
          </div>
          <h3>更多數據</h3>
          <div class="hrn1"></div>
          <div class="n1"></div>
          <table class="table-bordered" width="100%">
            <tr class="tta">
              <td class="unit" colspan="2" style="text-align: center;">
                <div class="unititle">劣勢與優勢</div>
              </td>
            </tr>
            <tr>
              <td class="tta1" style="width:30%">強勢(容易對抗)</td>
              <td class="tta4"><a href="Archer_Units.php">弓兵</a>、<a href="Pikeman.php">重裝長槍兵</a>。</td>
            </tr>
            <tr>
              <td class="tta1" style="width:30%">劣勢(不易對抗)</td>
              <td class="tta4">近戰單位、<a href="Siege_Weapons.php">攻城器單位</a>。</td>
            </tr>
            <tr class="tta">
              <td class="unit" colspan="2" style="text-align: center;">
                <div class="unititle">升級</div>
              </td>
            </tr>
            <tr>
              <td class="tta1" style="width:10%">攻擊力</td>
              <td class="tta4">
                <img class="gg" src="img/Fletching.jpg" width="30" alt=""><a href="Fletching.php">箭羽</a>(+1)
                <br>
                <img class="gg" src="img/Bodkinarrow.jpg" width="30" alt=""><a href="Bodkin_Arrow.php">錐狀箭頭</a>(+1)
                <br>
                <img class="gg" src="img/Bracer.jpg" width="30" alt=""><a href="Bracer.php">護腕</a>(+1)
                <br>
                <img class="gg" src="img/Chemistry.jpg" width="30" alt=""><a href="Chemistry.php">化學</a>(+1)
                <br>
                <img src="img/CastleAgeUnique.png" class="gg" alt=""><a href="Atlatl.php">擲矛器</a>(+1，<a href="Aztecs.php">阿茲特克</a>限定)</td>
            </tr>
            <tr>
              <td class="tta1" style="width:10%">攻擊範圍</td>
              <td class="tta4">
                <img class="gg" src="img/Fletching.jpg" width="30" alt=""><a href="Fletching.php">箭羽</a>(+1)
                <br>
                <img class="gg" src="img/Bodkinarrow.jpg" width="30" alt=""><a href="Bodkin_Arrow.php">錐狀箭頭</a>(+1)
                <br>
                <img class="gg" src="img/Bracer.jpg" width="30" alt=""><a href="Bracer.php">護腕</a>(+1)
                <br>
                <img src="img/CastleAgeUnique.png" class="gg" alt=""><a href="Atlatl.php">擲矛器</a>(+1，<a href="Aztecs.php">阿茲特克</a>限定)
                <br>
                <img src="img/CastleAgeUnique.png" class="gg" alt=""><a href="Yeomen.php">義勇騎兵</a>(+1,<a href="Britons.php">不列顛</a>限定)
                <br>
                <img src="img/CastleAgeUnique.png" class="gg" alt=""><a href="Andean_Sling.php">安地斯彈弓</a>:矛兵和投石手取消最小射程，<a href="Incas.php">印加</a>限定</td>
            </tr>
            <tr>
              <td class="tta1" style="width:10%">命中率</td>
              <td class="tta4">
                <img class="gg" src="img/Ballistics.jpg" width="30" alt=""><a href="Ballistic.php">彈道學</a>:對移動目標命中率上升
                <br>
                <img class="gg" src="img/Thumb_Ring.png" width="30" alt=""><a href="Thumb_Ring.php">拇指環</a>:命中率提升到100%</td>
            </tr>
            <tr>
              <td class="tta1" style="width:10%">防禦</td>
              <td class="tta4">
                <img class="gg" src="img/Paddedarcherarmor.jpg" width="30" alt=""><a href="Padded_Archer_Armor.php">弓兵布甲</a>(+1/+1)
                <br>
                <img class="gg" src="img/Leatherarcherarmor.jpg" width="30" alt=""><a href="Leather_Archer_Armor.php">弓兵皮甲</a>(+1/+1)
                <br>
                <img class="gg" src="img/Ringarcherarmor.jpg" width="30" alt=""><a href="Ring_Archer_Armor.php">弓兵環甲</a>(+1/+2)</td>
            </tr>
            <tr>
              <td class="tta1" style="width:10%">招降抵抗</td>
              <td class="tta4">
                <img class="gg" src="img/Faith.jpg" width="30" alt=""><a href="Faith.php">忠誠信仰</a>
                <br>
                <img class="gg" src="img/Heresy.png" width="30" alt=""><a href="Heresy.php">異端邪說</a> 
              </td>
            </tr>
            <tr>
              <td class="tta1" style="width:10%">生產速度</td>
              <td class="tta4">
                <img class="gg" src="img/Conscription.jpg" width="30" alt=""><a href="Conscription.php">徵兵制度</a>(+33%)</td>
            </tr>
          </table>
          <h3>剋帝王戰矛兵的單位</h3>
          <div class="hrn1"></div>
          <div class="n1">以下單位擁有對帝王戰矛兵的額外傷害，按造傷殺力排列。
            <div class="n1">
              <div class="row">
                <div class="col-md-6">
                  <div style="margin:12px 0px;"><strong>+15攻擊</strong>
                  </div>
                  <ul>
                    <li>
                      <img class="gg" src="img/CannonGalleonIcon.jpg" alt=""> <a href="Cannon_Galleon.php">火砲戰船</a>→
                      <img class="gg" src="img/CannonGalleonIcon.jpg"
                      alt=""> <a href="Elite_Cannon_Galleon.php">精銳火砲戰船</a>
                    </li>
                  </ul>
                  <div style="margin:12px 0px;"><strong>+10攻擊</strong>
                  </div>
                  <ul>
                    <li>
                      <img class="gg" src="img/HuskarlIcon.jpg" alt=""> <a href="Huskarl.php">精銳哥德衛隊</a>(<a href="Goths.php">哥德</a>限定)</li>
                  </ul>
                  <div style="margin:12px 0px;"><strong>+6攻擊</strong>
                  </div>
                  <ul>
                    <li>
                      <img class="gg" src="img/HuskarlIcon.jpg" alt=""> <a href="Huskarl.php">哥德衛隊</a>(<a href="Goths.php">哥德</a>限定)</li>
                  </ul>
                  <div style="margin:12px 0px;"><strong>+5攻擊</strong>
                  </div>
                  <ul>
                    <li>
                      <img class="gg" src="img/GenitourIcon.jpg" alt=""> <a href="Genitour.php">精銳標槍騎兵</a>(<a href="Berbers.php">柏柏</a>和同盟限定)</li>
                    <li>
                      <img class="gg" src="img/Imperialskirmishericon.png" alt=""> <a href="Imperial_Skirmisher.php">帝王戰矛兵</a>(<a href="Vietnamese.php">越南</a>和同盟限定)</li>
                  </ul>
                </div>
                <div class="col-md-6">
                  <div style="margin:12px 0px;"><strong>+4攻擊</strong>
                  </div>
                  <ul>
                    <li>
                      <img class="gg" src="img/EliteSkirmisherIcon.jpg" alt=""> <a href="Elite_Skirmisher.php">戰矛兵</a> 
                    </li>
                    <li>
                      <img class="gg" src="img/GenitourIcon.jpg" alt=""> <a href="Genitour.php">標槍騎兵</a>(<a href="Berbers.php">柏柏</a>和同盟限定)</li>
                  </ul>
                  <div style="margin:12px 0px;"><strong>+3攻擊</strong>
                  </div>
                  <ul>
                    <li>
                      <img class="gg" src="img/SkirmisherIcon.jpg" alt=""> <a href="Skirmisher.php">矛兵</a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>以下團隊加成可給以下單位對帝王戰矛兵有額外攻擊:
          <div class="n1">團隊加成:<a href="Persians.php">波斯</a>在團隊加成裡，給予以下單位+2額外攻擊vs<a href="Archer_Units.php">射手單位</a>。
            <div
            class="row">
              <div class="col-md-6">
                <div style="margin:12px 0px;"><strong>+2攻擊</strong>
                </div>
                <ul>
                  <li>
                    <img class="gg" src="img/KnightIcon.jpg" alt=""> <a href="Knight.php">騎士</a>→
                    <img class="gg" src="img/CavalierIcon.jpg"
                    alt=""> <a href="Cavalier.php">騎兵</a>→
                    <img class="gg" src="img/PaladinIcon.jpg"
                    alt=""> <a href="Paladin.php">遊俠</a> 
                </ul>
              </div>
          </div>
        </div>
        <h3>文明加成</h3>
        <div class="hrn1"></div>
        <div class="n1">
          <ul>
            <li><a href="Aztecs.php">阿茲特克</a>：生產快15%。</li>
            <li><a href="Byzantines.php">拜占庭</a>：成本便宜25%。</li>
            <li><a href="Chinese.php">中國</a>：對帝王戰矛兵有利的科技在城堡/帝王時代各便宜15%/20%。</li>
            <li><a href="Spanish.php">西班牙</a>：兵工廠升級科技免黃金</li>
            <li><a href="Vietnamese.php">越南</a>：帝王戰矛兵在帝王帝王時代的血量增加20%。免費<a href="Conscription.php">徵兵制度</a>。</li>
          </ul>
        </div>
        <h3>團隊加成</h3>
        <div class="hrn1"></div>
        <div class="n1">
          <ul>
            <li><a href="Berbers.php">柏柏</a>：<a href="Kasbah.php">聚居堡壘</a>能夠加速<a href="Conscription.php">徵兵制度</a>的研究時間(快25%)</li>
            <li><a href="Britons.php">不列顛</a>：帝王戰矛兵生產快20%。研發<a href="Thumb_Ring.php">拇指環</a>速度快20%。</li>
            <li><a href="Magyars.php">馬扎爾</a>：帝王戰矛兵<a href="">視野</a>+2。</li>
            <li><a href="Malians.php">馬利</a>：研究<a href="Chemistry.php">化學</a>和<a href="Ballistic.php">彈道學</a>快80%。</li>
            <li><a href="Saracens.php">薩拉森</a>：帝王戰矛兵對<a href="Armor_Class_Standard_Building.php">標準建築</a>攻擊+2。</li>
            <li><a href="Teutons.php">條頓</a>：帝王戰矛兵更不容易被招降。</li>
          </ul>
        </div>
        <h3>趣事</h3>
        <div class="hrn1"></div>
        <div class="n1">
          <ul>
            <li>帝王戰矛兵是所有單位最便宜的，與<a href="Armor_Class_Spearman.php">槍兵</a>相同，所需60資源，且不用黃金。</li>
            <li>攻擊只有2是所有陸上單位最低的，與<a href="Battering_Ram.php">輕型衝撞車</a>相同。</li>
            <li>血量30，與飛刀女獵手、爪刀勇士和弓兵相同，是遊戲中最少血量的單位。</li>
          </ul>
        </div>
      </div>
      <div class="col-md-4">
        <table class="table-bordered" width="100%">
          <tr>
            <td class="unit" colspan="2" style=" background-color:#3c763d;;">
              <div class="unititle2">帝王戰矛兵</div>
            </td>
          </tr>
          <tr>
            <td colspan="2">
              <div class="unititle">
                <img src="img/Imperialskirmishericon.png" width="45" alt="">
              </div>
            </td>
          </tr>
          <tr>
            <td class="unit" colspan="2" style=" background-color:#3c763d;;">
              <div class="unititle2">單位介紹</div>
            </td>
          </tr>
          <tr>
            <td class="unit3">類型</td>
            <td class="unit4"><a href="Archer_Units.php">射手</a>
            </td>
          </tr>
          <tr>
            <td class="unit3">文明</td>
            <td class="unit4"><a href="Vietnamese.php">越南</a>
            </td>
          </tr>
          <tr>
            <td class="unit3">時代</td>
            <td class="unit4">
              <img class="gg" src="img/Imperial-age-research.jpg" alt=""><a href="Imperial_Age.php">帝王時代</a>
            </td>
          </tr>
          <tr>
            <td class="unit3">訓練於</td>
            <td class="unit4">
              <img class="gg" width="25px" src="img/Archery-range-logo.jpg" alt=""><a href="Archery_Range.php">射箭場</a>
            </td>
          </tr>
          <tr>
            <td class="unit3">訓練時間</td>
            <td class="unit4">22秒</td>
          </tr>
          <tr>
            <td class="unit" colspan="2" style=" background-color:#3c763d;;">
              <div class="unititle2">成本</div>
            </td>
          </tr>
          <tr>
            <td class="unit3">木</td>
            <td class="unit4">35</td>
          </tr>
          <tr>
            <td class="unit3">肉</td>
            <td class="unit4">25</td>
          </tr>
          <tr>
            <td class="unit" colspan="2" style=" background-color:#3c763d;;">
              <div class="unititle2">數據</div>
            </td>
          </tr>
          <tr>
            <td class="unit3"><a href="Hit_points.php">血量</a>
            </td>
            <td class="unit4">35</td>
          </tr>
          <tr>
            <td class="unit3"><a href="Attack.php">攻擊</a>
            </td>
            <td class="unit4">4(遠程)</td>
          </tr>
          <tr>
            <td class="unit3">攻擊加成</td>
            <td class="unit4">+5對<a href="Armor_Class_Archer.php">弓箭手</a>
              <br>+3對<a href="Armor_Class_Spearman.php">槍兵類</a>
              <br>+3對<a href="Armor_Class_Cavalry_Archer.php">馬弓騎兵</a> 
            </td>
          </tr>
          <tr>
            <tr>
              <td class="unit3"><a href="Rate_of_Fire.php">射速</a>
              </td>
              <td class="unit4">3.05</td>
            </tr>
            <tr>
              <td class="unit3"><a href="Frame_delay.php">開火間隔</a>
              </td>
              <td class="unit4">5</td>
            </tr>
            <tr>
              <td class="unit3">射程</td>
              <td class="unit4">5</td>
            </tr>
            <tr>
              <tr>
                <td class="unit3">最小射程</td>
                <td class="unit4">1</td>
              </tr>
              <tr>
                <td class="unit3">命中率</td>
                <td class="unit4">95%</td>
              </tr>
              <tr>
                <td class="unit3">投射物速度</td>
                <td class="unit4">7</td>
              </tr>
              <tr>
                <td class="unit3"><a href="Armor_class.php">近防</a>
                </td>
                <td class="unit4">0</td>
              </tr>
              <tr>
                <td class="unit3"><a href="Armor_class.php">遠防</a>
                </td>
                <td class="unit4">5</td>
              </tr>
              <tr>
                <td class="unit3"><a href="Armor_class.php">防禦類型</a>
                </td>
                <td class="unit4"> <a href="Armor_Class_Archer.php">射手</a>
                </td>
              </tr>
              <tr>
                <td class="unit3"><a href="">跑速</a>
                </td>
                <td class="unit4">0.96</td>
              </tr>
              <tr>
                <td class="unit3"><a href="los.php">視野</a>
                </td>
                <td class="unit4">7</td>
              </tr>
              <tr>
                <td class="unit" colspan="2" style=" background-color:#3c763d;;">
                  <div class="unititle2"><a href="">單位演變</a>
                  </div>
                </td>
              </tr>
              <tr>
                <td class="unit3">從升級</td>
                <td class="unit4">
                  <img class="gg" src="img/EliteSkirmisherIcon.jpg" alt=""><a href="Elite_Skirmisher.php">戰矛兵</a>
                </td>
              </tr>
        </table>
      </div>
      <div class="col-md-12">
        <h3>圖庫</h3>
        <div class="hrn1"></div>
        <div class="n1">
          <div class="col-md-4"> <a href="img/Skirmishers_Line.png"><img class="imgn" class="hg" src="img/Skirmishers_Line.png" alt=""></a> 
            <p><em>矛兵系</em>
            </p>
          </div>
          <div class="col-md-4"> <a href="img/Imperial_Skirmisher_group.jpg"><img class="imgn" class="hg" src="img/Imperial_Skirmisher_group.jpg" alt=""></a> 
            <p><em>一群帝王戰矛兵</em>
            </p>
          </div>
        </div>
      </div>
    </div>
    </div>
    <!-- /container -->
    <?php include( "allunit.php"); ?>
    <?php include( "footn.php"); ?>
  </body>
</html>
