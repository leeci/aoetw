<!DOCTYPE html>
<html lang="zh-tw">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="description" content="">
    <meta name="author" content="">
    <link rel="icon" href="favicon.ico">
    <title>Aoetw</title>
    <style>
      body {            padding-top: 0px;            font-family: "微軟正黑體";            background-color: #f5f5f5;            font-size: 18px;        }
    </style>
  </head>
  <body>
    <?php include( "head.php"); ?>
    <!-- Main jumbotron for a primary marketing message or call to action
    -->
    <div class="container">
      <!-- Example row of columns -->
      <div class="row" style=" margin-top: 50px;">
        <div class="col-md-12">
          <h1>步兵單位</h1>
          <div class="hrn1"></div>
        </div>
        <div class="col-md-12">
          <div class="n1">
            <p>步兵單位是遊戲中四類軍事單位之一，另外三種分別是<a href="Cavalry_Units.php">騎兵單位</a>、<a href="Archer_Units.php">弓兵單位</a>與
              <a
              href="Siege_Weapon_Unit.php">攻城武器單位</a>。</p>
          </div>
          <h3>概要</h3>
          <div class="hrn1"></div>
          <div class="n1">
            <ul>
              <li>步兵單位通常是生命值較低，速度也慢，不過是非常重要的單位，因為他相當便宜也容易量產，且早期就可以訓練。</li>
              <li>普通的步兵單位在軍營裡訓練，民兵是第一個可以訓練的單位也是黑暗時代唯一可生產的軍事單位。</li>
              <li>特殊單位的步兵在城堡裡面訓練，但有兩個例外，<a href="Huskarl.php">哥德衛隊</a>在研發無政府狀態後可以在軍營訓練，而
                <a
                href="Condottiero.php">傭兵</a>只要是義大利及其同盟也可以在帝王時代時在軍營訓練。</li>
              <li>所有的步兵都需要黃金和食物訓練，而槍兵系是唯一的例外，他需要食物與木材。雖然大部分的步兵懼怕弓兵單位與騎兵單位，但其實仍有一些好處:第一:他們成本很低，尤其是黃金，表示可以用較少的資源去換傷亡比。第二:
                <a
                href="Archer.php">弓箭手</a>通常可以配合<a href="Skirmisher.php">矛兵</a>，而騎兵則是搭配<a href="Pikeman.php">槍兵</a>。</li>
              <li>由於速度很慢又是近戰，步兵很怕僧侶。</li>
              <li>擅長對付步兵的單位是:<a href="Hand_Cannoneer.php">火槍兵</a>、投石手、羽毛箭射手、條頓武士、豹勇士與聖騎兵</li>
            </ul>
          </div>
          <h3>步兵文明</h3>
          <div class="hrn1"></div>
          <div class="n1">
            <p>許多文明依賴強大的步兵，<a href="Aztecs.php">阿茲特克人</a>，<a href="Burmese.php">緬甸人</a>，
              <a
              href="Japanese.php">日本人</a>，<a href="Slavs.php">斯拉夫人</a>和<a href="Vikings.php">維京人</a>可以說是所有文明中最強大的步兵。
                <a
                href="Celts.php">賽爾特</a>、<a href="Goths.php">哥德</a>、<a href="Incas.php">印加</a>、<a href="Malians.php">馬利</a>與
                  <a
                  href="Teutons.php">條頓</a>也是優秀的步兵文明。而<a href="Incas.php">印加</a>是唯一有所有步兵單位和技術的文明。</p>
          </div>
          <h3>文明加成</h3>
          <div class="hrn1"></div>
          <div class="n1">
            <p>以下列出步兵有加成的文明</p>
            <ul>
              <li><a href="Burmese.php">緬甸</a>:步兵單位在封建/城堡/帝王各有+1/+2/+3的攻擊。</li>
              <li><a href="Byzantines.php">拜占庭</a>:<a href="Pikeman.php">長槍兵</a>便宜25%。</li>
              <li><a href="Celts.php">賽爾特</a>:步兵移動快25%。</li>
              <li><a href="Ethiopians.php">衣索比亞</a>:槍兵係免費升級。</li>
              <li><a href="Goths.php">哥德</a>:步兵便宜35%，步兵對<a href="Armor_Class_Standard_Building.php">標準建築有</a>+1攻擊。</li>
              <li><a href="Japanese.php">日本</a>:從封建時代開始，步兵攻擊速度快25%。</li>
              <li><a href="Malians.php">馬利</a>:兵營的步兵單位從封建/城堡/帝王各有+1/+2/+3的遠防。</li>
              <li><a href="Slavs.php">斯拉夫</a>:追蹤技術是免費的。</li>
              <li><a href="Vikings.php">維京</a>:步兵在封建/城堡/帝王各有+10%/+15%/+20%的生命值。</li>
            </ul>
          </div>
          <h3>團隊加成</h3>
          <div class="hrn1"></div>
          <div class="n1">
            <p>以下列出步兵的團隊加成</p>
            <ul>
              <li><a href="Goths.php">哥德</a>:軍營工作效率快20%</li>
              <li><a href="Italians.php">義大利</a>:<a href="Condottiero.php">傭兵</a>可在軍營訓練</li>
            </ul>
          </div>
          <h3>步兵單位</h3>
          <div class="hrn1"></div>
          <div class="n1">
            <table class="table-bordered" width="100%">
              <tr class="tta">
                <td width="19%">步兵單位</td>
                <td width="9%">生命值</td>
                <td width="9%">攻擊</td>
                <td width="9%">防禦</td>
                <td width="9%">射程</td>
                <td width="9%">攻擊速度</td>
                <td width="9%">視野</td>
                <td width="9%">速度</td>
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/MilitiaIcon.jpg" alt=""><a href="Militia.php">民兵</a>
                </td>
                <td class="tta3">40</td>
                <td class="tta3">4</td>
                <td class="tta3">0/1</td>
                <td class="tta3">0</td>
                <td class="tta3">2.03</td>
                <td class="tta3">4</td>
                <td class="tta3">0.9</td>
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/ManAtArmsIcon.jpg" width="22" alt=""><a href="Man-at-Arms.php">裝甲步兵</a>
                </td>
                <td class="tta3">45</td>
                <!--生命值!-->
                <td class="tta3">6</td>
                <!--攻擊!-->
                <td class="tta3">0/1</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">2.03</td>
                <!--射速!-->
                <td class="tta3">4</td>
                <!--視野!-->
                <td class="tta3">0.9</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/LongswordsManIcon.jpg" alt=""><a href="Long_Swordsman.php">長劍兵</a>
                </td>
                <td class="tta3">60</td>
                <!--生命值!-->
                <td class="tta3">9</td>
                <!--攻擊!-->
                <td class="tta3">0/1</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">2.03</td>
                <!--射速!-->
                <td class="tta3">4</td>
                <!--視野!-->
                <td class="tta3">0.9</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/2HandedswordsmanIcon.jpg" alt=""><a href="Two-Handed_Swordsman.php">雙手劍兵</a>
                </td>
                <td class="tta3">60</td>
                <!--生命值!-->
                <td class="tta3">12</td>
                <!--攻擊!-->
                <td class="tta3">0/1</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">2.03</td>
                <!--射速!-->
                <td class="tta3">5</td>
                <!--視野!-->
                <td class="tta3">0.9</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/ChampionIcon.jpg" alt=""><a href="Champion.php">劍兵勇士</a>
                </td>
                <td class="tta3">70</td>
                <!--生命值!-->
                <td class="tta3">13</td>
                <!--攻擊!-->
                <td class="tta3">0/1</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">2.03</td>
                <!--射速!-->
                <td class="tta3">5</td>
                <!--視野!-->
                <td class="tta3">0.9</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/SpearmanIcon.jpg" alt=""><a href="Spearman.php">長槍兵</a>
                </td>
                <td class="tta3">45</td>
                <!--生命值!-->
                <td class="tta3">3</td>
                <!--攻擊!-->
                <td class="tta3">0/0</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">3.05</td>
                <!--射速!-->
                <td class="tta3">4</td>
                <!--視野!-->
                <td class="tta3">1</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/PikemanIcon.jpg" alt=""><a href="Pikeman.php">重裝長槍兵</a>
                </td>
                <td class="tta3">55</td>
                <!--生命值!-->
                <td class="tta3">4</td>
                <!--攻擊!-->
                <td class="tta3">0/0</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">3.05</td>
                <!--射速!-->
                <td class="tta3">4</td>
                <!--視野!-->
                <td class="tta3">1</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/HalberdierIcon.jpg" alt=""><a href="Halberdier.php">戟兵</a>
                </td>
                <td class="tta3">60</td>
                <!--生命值!-->
                <td class="tta3">6</td>
                <!--攻擊!-->
                <td class="tta3">0/0</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">3.05</td>
                <!--射速!-->
                <td class="tta3">4</td>
                <!--視野!-->
                <td class="tta3">1</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/EagleScoutIcon.jpg" alt=""><a href="Eagle_Scout.php">鷹斥侯</a>
                </td>
                <td class="tta3">50</td>
                <!--生命值!-->
                <td class="tta3">4</td>
                <!--攻擊!-->
                <td class="tta3">0/2</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">2.03</td>
                <!--射速!-->
                <td class="tta3">6</td>
                <!--視野!-->
                <td class="tta3">1.1</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/EagleWarriorIcon.jpg" alt=""><a href="Eagle_Warrior.php">鷹勇士</a>
                </td>
                <td class="tta3">55</td>
                <!--生命值!-->
                <td class="tta3">7</td>
                <!--攻擊!-->
                <td class="tta3">0/3</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">2.03</td>
                <!--射速!-->
                <td class="tta3">5</td>
                <!--視野!-->
                <td class="tta3">1.15</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/EliteEagleWarriorIcon.jpg" alt=""><a href="Eagle_Warrior.php">精銳鷹勇士</a>
                </td>
                <td class="tta3">60</td>
                <!--生命值!-->
                <td class="tta3">9</td>
                <!--攻擊!-->
                <td class="tta3">0/4</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">2.03</td>
                <!--射速!-->
                <td class="tta3">6</td>
                <!--視野!-->
                <td class="tta3">1.3</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/CondottieroNewIcon.jpg" alt=""><a href="Condottiero.php">傭兵</a>
                </td>
                <td class="tta3">80</td>
                <!--生命值!-->
                <td class="tta3">9</td>
                <!--攻擊!-->
                <td class="tta3">1/0</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">1.93</td>
                <!--射速!-->
                <td class="tta3">6</td>
                <!--視野!-->
                <td class="tta3">1.2</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/WBRSK.jpg" alt=""><a href="Woad_Raider.php">菘藍武士</a>
                </td>
                <td class="tta3">65</td>
                <!--生命值!-->
                <td class="tta3">8</td>
                <!--攻擊!-->
                <td class="tta3">0/1</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">2.03</td>
                <!--射速!-->
                <td class="tta3">3</td>
                <!--視野!-->
                <td class="tta3">1.2</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/WBRSK.jpg" alt=""><a href="Woad_Raider.php">精銳菘藍武士</a>
                </td>
                <td class="tta3">80</td>
                <!--生命值!-->
                <td class="tta3">13</td>
                <!--攻擊!-->
                <td class="tta3">0/1</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">2.03</td>
                <!--射速!-->
                <td class="tta3">5</td>
                <!--視野!-->
                <td class="tta3">1.2</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/TAXEM.jpg" alt=""><a href="Throwing_Axeman.php">擲斧兵</a>
                </td>
                <td class="tta3">60</td>
                <!--生命值!-->
                <td class="tta3">7</td>
                <!--攻擊!-->
                <td class="tta3">0/0</td>
                <!--防禦!-->
                <td class="tta3">3</td>
                <!--射程!-->
                <td class="tta3">2.03</td>
                <!--射速!-->
                <td class="tta3">5</td>
                <!--視野!-->
                <td class="tta3">1</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/TAXEM.jpg" alt=""><a href="Throwing_Axeman.php">精銳擲斧兵</a>
                </td>
                <td class="tta3">70</td>
                <!--生命值!-->
                <td class="tta3">8</td>
                <!--攻擊!-->
                <td class="tta3">1/0</td>
                <!--防禦!-->
                <td class="tta3">4</td>
                <!--射程!-->
                <td class="tta3">1.83</td>
                <!--射速!-->
                <td class="tta3">6</td>
                <!--視野!-->
                <td class="tta3">1</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/HuskarlIcon.jpg" alt=""><a href="Huskarl.php">哥德衛隊</a>
                </td>
                <td class="tta3">60</td>
                <!--生命值!-->
                <td class="tta3">10</td>
                <!--攻擊!-->
                <td class="tta3">0/6</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">1.73</td>
                <!--射速!-->
                <td class="tta3">3</td>
                <!--視野!-->
                <td class="tta3">1.05</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/HuskarlIcon.jpg" alt=""><a href="Huskarl.php">精銳哥德衛隊</a>
                </td>
                <td class="tta3">70</td>
                <!--生命值!-->
                <td class="tta3">12</td>
                <!--攻擊!-->
                <td class="tta3">0/8</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">2.03</td>
                <!--射速!-->
                <td class="tta3">5</td>
                <!--視野!-->
                <td class="tta3">1.05</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/SMURI.jpg" alt=""><a href="Samurai.php">日本武士</a>
                </td>
                <td class="tta3">60</td>
                <!--生命值!-->
                <td class="tta3">8</td>
                <!--攻擊!-->
                <td class="tta3">1/1</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">1.93</td>
                <!--射速!-->
                <td class="tta3">4</td>
                <!--視野!-->
                <td class="tta3">1</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/SMURI.jpg" alt=""><a href="Samurai.php">精銳日本武士</a>
                </td>
                <td class="tta3">80</td>
                <!--生命值!-->
                <td class="tta3">12</td>
                <!--攻擊!-->
                <td class="tta3">1/1</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">1.93</td>
                <!--射速!-->
                <td class="tta3">5</td>
                <!--視野!-->
                <td class="tta3">1</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/Crusade_avatar.png" alt=""><a href="Teutonic_Knight.php">條頓武士</a>
                </td>
                <td class="tta3">80</td>
                <!--生命值!-->
                <td class="tta3">12</td>
                <!--攻擊!-->
                <td class="tta3">5/2</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">2.03</td>
                <!--射速!-->
                <td class="tta3">3</td>
                <!--視野!-->
                <td class="tta3">0.7</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/Crusade_avatar.png" alt=""><a href="Teutonic_Knight.php">精銳條頓武士</a>
                </td>
                <td class="tta3">100</td>
                <!--生命值!-->
                <td class="tta3">17</td>
                <!--攻擊!-->
                <td class="tta3">10/2</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">2.13</td>
                <!--射速!-->
                <td class="tta3">5</td>
                <!--視野!-->
                <td class="tta3">0.7</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/VBRSK.jpg" alt=""><a href="Berserk.php">狂戰士</a>
                </td>
                <td class="tta3">54</td>
                <!--生命值!-->
                <td class="tta3">9</td>
                <!--攻擊!-->
                <td class="tta3">0/1</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">2.13</td>
                <!--射速!-->
                <td class="tta3">3</td>
                <!--視野!-->
                <td class="tta3">1.05</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/VBRSK.jpg" alt=""><a href="Berserk.php">精銳狂戰士</a>
                </td>
                <td class="tta3">62</td>
                <!--生命值!-->
                <td class="tta3">14</td>
                <!--攻擊!-->
                <td class="tta3">2/1</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">1.83</td>
                <!--射速!-->
                <td class="tta3">5</td>
                <!--視野!-->
                <td class="tta3">1.05</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/JaguarWarriorIcon.jpg" alt=""><a href="Jaguar_Warrior.php">豹勇士</a>
                </td>
                <td class="tta3">50</td>
                <!--生命值!-->
                <td class="tta3">10</td>
                <!--攻擊!-->
                <td class="tta3">1/1</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">1.83</td>
                <!--射速!-->
                <td class="tta3">3</td>
                <!--視野!-->
                <td class="tta3">1</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/JaguarWarriorIcon.jpg" alt=""><a href="Jaguar_Warrior.php">精銳豹勇士</a>
                </td>
                <td class="tta3">75</td>
                <!--生命值!-->
                <td class="tta3">12</td>
                <!--攻擊!-->
                <td class="tta3">2/1</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">1.93</td>
                <!--射速!-->
                <td class="tta3">5</td>
                <!--視野!-->
                <td class="tta3">1</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/Kamayuk_Icon.jpg" alt=""><a href="Kamayuk.php">槍隊長</a>
                </td>
                <td class="tta3">60</td>
                <!--生命值!-->
                <td class="tta3">7</td>
                <!--攻擊!-->
                <td class="tta3">0/0</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">1.93</td>
                <!--射速!-->
                <td class="tta3">4</td>
                <!--視野!-->
                <td class="tta3">1</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/Kamayuk_Icon.jpg" alt=""><a href="Kamayuk.php">精銳槍隊長</a>
                </td>
                <td class="tta3">80</td>
                <!--生命值!-->
                <td class="tta3">7</td>
                <!--攻擊!-->
                <td class="tta3">1/0</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">2.5</td>
                <!--射速!-->
                <td class="tta3">5</td>
                <!--視野!-->
                <td class="tta3">1</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/ShotelIcon.png" alt=""><a href="Shotel_Warrior.php">彎刀勇士</a>
                </td>
                <td class="tta3">40</td>
                <!--生命值!-->
                <td class="tta3">16</td>
                <!--攻擊!-->
                <td class="tta3">0/0</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">2.03</td>
                <!--射速!-->
                <td class="tta3">3</td>
                <!--視野!-->
                <td class="tta3">1.2</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/ShotelIcon.png" alt=""><a href="Shotel_Warrior.php">精銳彎刀勇士</a>
                </td>
                <td class="tta3">50</td>
                <!--生命值!-->
                <td class="tta3">18</td>
                <!--攻擊!-->
                <td class="tta3">0/1</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">2.03</td>
                <!--射速!-->
                <td class="tta3">3</td>
                <!--視野!-->
                <td class="tta3">1.2</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/GbetoIcon.png" alt=""><a href="Gbeto.php">飛刀女獵手</a>
                </td>
                <td class="tta3">30</td>
                <!--生命值!-->
                <td class="tta3">10</td>
                <!--攻擊!-->
                <td class="tta3">0/0</td>
                <!--防禦!-->
                <td class="tta3">5</td>
                <!--射程!-->
                <td class="tta3">2.03</td>
                <!--射速!-->
                <td class="tta3">6</td>
                <!--視野!-->
                <td class="tta3">1.25</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/GbetoIcon.png" alt=""><a href="Gbeto.php">精銳飛刀女獵手</a>
                </td>
                <td class="tta3">45</td>
                <!--生命值!-->
                <td class="tta3">13</td>
                <!--攻擊!-->
                <td class="tta3">0/0</td>
                <!--防禦!-->
                <td class="tta3">6</td>
                <!--射程!-->
                <td class="tta3">2.03</td>
                <!--射速!-->
                <td class="tta3">7</td>
                <!--視野!-->
                <td class="tta3">1.25</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/Karambitwarrioricon.png" alt=""><a href="Karambit_Warrior.php">爪刀勇士</a>
                </td>
                <td class="tta3">30</td>
                <!--生命值!-->
                <td class="tta3">6</td>
                <!--攻擊!-->
                <td class="tta3">0/1</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">2.03</td>
                <!--射速!-->
                <td class="tta3">3</td>
                <!--視野!-->
                <td class="tta3">1.2</td>
                <!--速度!-->
              </tr>
              <tr>
                <td class="tta1">
                  <img class="gg" src="img/Karambitwarrioricon.png" alt=""><a href="Karambit_Warrior.php">精銳爪刀勇士</a>
                </td>
                <td class="tta3">40</td>
                <!--生命值!-->
                <td class="tta3">7</td>
                <!--攻擊!-->
                <td class="tta3">1/1</td>
                <!--防禦!-->
                <td class="tta3">0</td>
                <!--射程!-->
                <td class="tta3">2.03</td>
                <!--射速!-->
                <td class="tta3">3</td>
                <!--視野!-->
                <td class="tta3">1.2</td>
                <!--速度!-->
              </tr>
            </table>
          </div>
          <h3>相關科技</h3> 
          <div class="hrn1"></div>
          <div class="n1">
            <table class="table-bordered" width="100%">
              <tr>
                <td class="tunitcc" style="width: 15%;">科技</td>
                <td class="tunitcc" style="width: 10%;">時代</td>
                <td class="tunitcc" style="width: 15%;">成本</td>
                <td class="tunitcc">影響</td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img class="gg" src="img/Tracking.jpg" alt=""><a href="Tracking.php">追縱技術</a>
                </td>
                <td class="tunitc4">封建</td>
                <td class="tunitc5">50F</td>
                <td class="tunitc6">步兵+2視野</td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img class="gg" src="img/Squires.jpg" alt=""><a href="Squires.php">護衛</a>
                </td>
                <td class="tunitc4">城堡</td>
                <td class="tunitc5">100F</td>
                <td class="tunitc6">步兵速度+10%</td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img class="gg" src="img/Arson.jpg" alt=""><a href="Arson.php">縱火</a>
                </td>
                <td class="tunitc4">城堡</td>
                <td class="tunitc5">150F 50G</td>
                <td class="tunitc6">步兵+2攻擊vs<a href="Armor_Class_Standard_Building.php">標準建築</a>
                </td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img class="gg" src="img/Forging.jpg" alt=""><a href="Forging.php">鍛造</a>
                </td>
                <td class="tunitc4">封建</td>
                <td class="tunitc5">150F</td>
                <td class="tunitc6">步兵攻擊+1。</td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img class="gg" src="img/Ironcasting.jpg" alt=""><a href="Iron_Casting.php">鑄造</a>
                </td>
                <td class="tunitc4">城堡</td>
                <td class="tunitc5">220F 120G</td>
                <td class="tunitc6">步兵攻擊+1。</td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img class="gg" src="img/Blastfurnace.jpg" alt=""><a href="Blast_Furnace.php">鼓風爐</a>
                </td>
                <td class="tunitc4">帝王</td>
                <td class="tunitc5">275F 225G</td>
                <td class="tunitc6">步兵攻擊+2。</td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img class="gg" src="img/Scalemailarmor.jpg" alt=""><a href="Scale_Mail_Armor.php">步兵鱗甲</a>
                </td>
                <td class="tunitc4">封建</td>
                <td class="tunitc5">100F</td>
                <td class="tunitc6">步兵護甲+1/+1。</td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img class="gg" src="img/Chainmailarmor.jpg" alt=""><a href="Chain_Mail_Armor.php">步兵鎖甲</a>
                </td>
                <td class="tunitc4">城堡</td>
                <td class="tunitc5">200F 100G</td>
                <td class="tunitc6">步兵護甲+1/+1。</td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img class="gg" src="img/Platemailarmor.jpg" alt=""><a href="Plate_Mail_Armor.php">步兵板甲</a>
                </td>
                <td class="tunitc4">帝王</td>
                <td class="tunitc5">300F 150G</td>
                <td class="tunitc6">步兵護甲+1/+2。</td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img class="gg" src="img/Herbalmedicine.png" alt=""><a href="Herbal_Medicine.php">草藥學</a>
                </td>
                <td class="tunitc4">城堡</td>
                <td class="tunitc5">350G</td>
                <td class="tunitc6">駐紮單位自身治療速度變為4倍。</td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img class="gg" src="img/Heresy.png" alt=""><a href="Heresy.php">異端邪說</a>
                </td>
                <td class="tunitc4">城堡</td>
                <td class="tunitc5">1000G</td>
                <td class="tunitc6">被招降的單位/建築物立即死亡/毀壞)。</td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img class="gg" src="img/Faith.jpg" alt=""><a href="Faith.php">忠誠信仰</a>
                </td>
                <td class="tunitc4">帝王</td>
                <td class="tunitc5">750F 1000G</td>
                <td class="tunitc6">單位對僧侶(傳教士)的招降更具抵抗力。</td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img class="gg" src="img/Conscription.jpg" width="30" alt=""><a href="Conscription.php">徵兵制度</a>
                </td>
                <td class="tunitc4">帝王</td>
                <td class="tunitc5">150F 150G</td>
                <td class="tunitc6">軍營、靶場、馬廄、城堡訓練部隊速度+33%。</td>
              </tr>
            </table>
          </div>
           <h3>相關特殊科技</h3> 
          <div class="hrn1"></div>
          <div class="n1">
            <table class="table-bordered table-striped" width="100%">
              <tr>
                <td class="tunitcc" style="width: 15%;">文明</td>
                <td class="tunitcc" style="width: 10%;">特殊科技</td>
                <td class="tunitcc" style="width: 15%;">花費</td>
                <td class="tunitcc">影響</td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img src="img/CastleAgeUnique.png" class="gg" alt=""><a href="Berbers.php">柏柏</a>
                </td>
                <td class="tunitc4"><a href="Kasbah.php">聚居堡壘</a>
                </td>
                <td class="tunitc5">250F 250G</td>
                <td class="tunitc6">團隊的城堡工作效率快25%</td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img src="img/CastleAgeUnique.png" class="gg" style="margin: 2px;" alt=""><a href="Eithopians.php">衣索比亞</a>
                </td>
                <td class="tunitc4"><a href="Royal_Heirs.php">皇族血脈</a>
                </td>
                <td class="tunitc5">300F 300G</td>
                <td class="tunitc6"><a href="Shotel_Warrior.php">彎刀勇士</a>訓練速度加倍(8秒變4秒)。</td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img src="img/CastleAgeUnique.png" class="gg" style="margin: 2px;" alt=""><a href="Goths.php">哥德</a>
                </td>
                <td class="tunitc4"><a href="Anarchy.php">無政府狀態</a>
                </td>
                <td class="tunitc5">450F 250G</td>
                <td class="tunitc6">兵營可以訓練<a href="Huskarl.php">哥德衛隊</a>。</td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img src="img/CastleAgeUnique.png" class="gg" style="margin: 2px;" alt=""><a href="Vikings.php">維京</a>
                </td>
                <td class="tunitc4"><a href="Chieftains.php">酋長</a>
                </td>
                <td class="tunitc5">400F 300G</td>
                <td class="tunitc6">步兵對騎兵攻擊+5，對<a href="Camel.php">駱駝騎兵</a>攻擊+2。</td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img src="img/Unique-tech.jpg" class="gg" style="margin: 2px;" alt=""><a href="Aztecs.php">阿茲特克</a>
                </td>
                <td class="tunitc4"><a href="Garland_Wars.php">榮冠戰爭</a>
                </td>
                <td class="tunitc5">450F 750G</td>
                <td class="tunitc6">步兵攻擊+4。</td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img src="img/Unique-tech.jpg" class="gg" style="margin: 2px;" alt=""><a href="Franks.php">法蘭克</a>
                </td>
                <td class="tunitc4"><a href="Bearded_Axe.php">倒鉤斧</a>
                </td>
                <td class="tunitc5">400F 400G</td>
                <td class="tunitc6"><a href="Throwing%20Axeman.php">擲斧兵</a>攻擊距離+1。</td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img src="img/Unique-tech.jpg" class="gg" style="margin: 2px;" alt=""><a href="Goths.php">哥德</a>
                </td>
                <td class="tunitc4"><a href="Perfusion.php">灌注士氣</a>
                </td>
                <td class="tunitc5">400W 600G</td>
                <td class="tunitc6">兵營工作速度加倍。</td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img src="img/Unique-tech.jpg" class="gg" style="margin: 2px;" alt=""><a href="Incas.php">印加</a>
                </td>
                <td class="tunitc4"><a href="Couriers.php">信使</a>
                </td>
                <td class="tunitc5">400F 200G</td>
                <td class="tunitc6"><a href="Kamayuk.php">槍隊長</a>、<a href="Slinger.php">投石手</a>和<a href="Eagle_Warrior.php">鷹勇士</a>的護甲+1/+2。</td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img src="img/Unique-tech.jpg" class="gg" style="margin: 2px;" alt=""><a href="Malay.php">馬來</a>
                </td>
                <td class="tunitc4"><a href="Forced_Levy.php">強制徵兵</a>
                </td>
                <td class="tunitc5">1000F 600G</td>
                <td class="tunitc6">軍營步兵單位訓練不需花食物。</td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img src="img/Unique-tech.jpg" class="gg" style="margin: 2px;" alt=""><a href="Mayans.php">馬雅</a>
                </td>
                <td class="tunitc4"><a href="Ei_Dorado.php">黃金國</a>
                </td>
                <td class="tunitc5">750F 450G</td>
                <td class="tunitc6"><a href="Eagle_Warrior.php">鷹勇士</a>+40 HP。</td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img src="img/Unique-tech.jpg" class="gg" style="margin: 2px;" alt=""><a href="Slavs.php">斯拉夫</a>
                </td>
                <td class="tunitc4"><a href="Druzhina.php">王家侍從</a>
                </td>
                <td class="tunitc5">1200F 500G</td>
                <td class="tunitc6">步兵單位有踐踏傷害。</td>
              </tr>
              <tr>
                <td class="tunitc6">
                  <img src="img/Unique-tech.jpg" class="gg" style="margin: 2px;" alt=""><a href="Vikings.php">維京</a>
                </td>
                <td class="tunitc4"><a href="Berserkergang.php">狂戰士幫</a>
                </td>
                <td class="tunitc5">450F 400G</td>
                <td class="tunitc6"><a href="Bersek.php">狂戰士</a>回血速度提升(每分鐘回20 HP變每分鐘回40 HP)。</td>
              </tr>
            </table>
            <div class="thanks">感謝! 此頁面由賜齡協助編輯</div>
          </div>
        </div>
        <div class="col-md-12">
           <h3>圖庫</h3>
          <div class="hrn1"></div>
          <div class="n1">
            <div class="col-md-4"> <a href="img/Infantryunitsbarracks.png"><img class="imgn" class="hg" width="180" src="img/Infantryunitsbarracks.png" alt=""></a> 
              <div
              class="lightbox-caption" style="width:220px" ;>
                <p><em>所有軍營裡可訓練的步兵單位。</em>
                </p>
            </div>
          </div>
          <div class="col-md-4"> <a href="img/Infantryunitscastle.png"><img class="imgn" class="hg" width="200" src="img/Infantryunitscastle.png" alt=""></a> 
            <div
            class="lightbox-caption" style="width:220px" ;>
              <p><em>所有城堡裡可訓練的步兵單位。</em>
              </p>
          </div>
        </div>
      </div>
    </div>
    </div>
    </div>
    <!-- /container -->
    <?php include( "allunit2.php"); ?>
    <?php include( "foot.php"); ?>
  </body>
</html>